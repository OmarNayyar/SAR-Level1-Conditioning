...
def main() -> None:
    ...
    parser = build_bundle_arg_parser(
        description="Run Bundle D: PnP-ADMM additive cleanup plus optional Speckle2Void wrapper.",
        default_config_path="configs/bundle_d.yaml",
    )
    ...
    dataset_name = args.dataset or dataset_cfg.get("name", "hrsid")
    ...
    if decision.action == "reuse":
        ...
        write_artifact_manifest(
            ...
            metadata={"bundle_name": "bundle_d", "dataset": dataset_name},
    ...
    summary = run_bundle_d(records, dataset_name=dataset_name, config=config, output_root=output_root)
    write_artifact_manifest(
        output_root,
        artifact_kind="bundle_run",
        identity=identity,
        ...
        metadata={"bundle_name": "bundle_d", "dataset": dataset_name, "policy": describe_policy(policy)},
