...
def main() -> None:
    ...
    parser = build_bundle_arg_parser(
        description="Run Bundle B: destriping plus MuLoG-style log-domain despeckling.",
        default_config_path="configs/bundle_b.yaml",
    )
    ...
    if decision.action == "reuse":
        ...
        write_artifact_manifest(
            ...
            metadata={"bundle_name": "bundle_b", "dataset": dataset_name},
    ...
    summary = run_bundle_b(records, dataset_name=dataset_name, config=config, output_root=output_root)
    write_artifact_manifest(
        output_root,
        artifact_kind="bundle_run",
        identity=identity,
        ...
        metadata={"bundle_name": "bundle_b", "dataset": dataset_name, "policy": describe_policy(policy)},
